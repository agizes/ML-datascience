...
```{r clean testing dataset echo=FALSE}
dim(testing)
testing <- testing[,-(1:7)]
...
RFmodelFit
```
We already see from this output that the model is quite accurate (0.9922846  for 2 variables randomly sampled as candidates at each split).
...
BoostmodelFit
```
We see hte accuracy is lower than the random forest model. Let's see it detail:
...
So the boosting model has a 0.9594 accuracy versus the 0.9922846 accuracy of the random forest one. Therefore, the preferred model would be the random forest.
...
```{r Prediction testing}
RFTest <- predict(RFmodelFit, trainingDS)
BoostTtest<- predict(BoostmodelFit, trainingDS)
table(RFTest,BoostTtest)
```
